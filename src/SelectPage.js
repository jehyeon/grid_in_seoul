...
class SelectPage extends Component {
  ...
  handlePageSelect = (e) => {
    ...
    if (e.target.getAttribute('type') === 'left' && index > 0) {
      ...
      e.target.getAttribute('type') === 'right' &&
    ...
    return (
      <div className="wrapper">
        <div className="select_view">
          <div className="left" type="left" onClick={this.handlePageSelect} />
          <div className="text">{now_page}</div>
          <div className="right" type="right" onClick={this.handlePageSelect} />
        </div>
        <ul>{year_select}</ul>
      </div>
    );
